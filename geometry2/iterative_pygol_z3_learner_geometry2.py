...
from z3 import Solver, Real, Int, And, Or, sat, Optimize, Sum, If, set_param
...
class IterativePyGolZ3Learner(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        ...
        # Set Z3 timeout: 5 minutes per Z3 operation (prevents hours-long hangs but allows complex optimizations)
        # This applies to all Optimize() and Solver() calls
        # 5 minutes is reasonable for complex relational constraints while still preventing indefinite hangs
        self.z3_timeout_ms = 300000  # 5 minutes (300 seconds) in milliseconds
        # Set global Z3 timeout (applies to all Z3 operations)
        set_param('timeout', self.z3_timeout_ms)
    ...
    y: np.ndarray,
     iteration: int = 0):
        ...
        import time
        pygol_start_time = time.time()
        
        if self.verbose:
            print(f"\n[Iteration {iteration}] Learning rules with PyGol...")
            print(f"  [PyGol Settings] max_literals={self.max_literals}, timeout={self.pygol_timeout}s")
        ...
        if self.verbose:
            print(f"  [PyGol Input] {len(Training_pos)} positive examples, {len(Training_neg)} negative examples")
    ...
    f"Bottom clause generation failed, using direct examples: {e}")
        ...
        pygol_time = time.time() - pygol_start_time
        
        if not hasattr(
    pygol_model,
     'hypothesis') or not pygol_model.hypothesis:
            if self.verbose:
                print(f"[PyGol Result] No rules learned (time: {pygol_time:.1f}s)")
            return []
        
        num_pygol_rules = len(pygol_model.hypothesis)
        if self.verbose:
            print(f"[PyGol Result] Learned {num_pygol_rules} rules in {pygol_time:.1f}s")
            if num_pygol_rules > 0:
                print(f"  [PyGol Rules] First 3 rules:")
                for i, rule in enumerate(pygol_model.hypothesis[:3], 1):
                    print(f"    {i}. {rule}")
                if num_pygol_rules > 3:
                    print(f"    ... and {num_pygol_rules - 3} more")
            else:
                print(f"  [WARNING] PyGol returned 0 rules - arithmetic learning will be the only source")
        
        # Convert to internal format
        converted_rules = self._convert_pygol_rules(pygol_model.hypothesis, X)
        if self.verbose:
            print(f"  [PyGol Conversion] Converted {num_pygol_rules} PyGol rules to {len(converted_rules)} internal rules")
    ...
    def _optimize_threshold_smt(self, rule: Dict, X: pd.DataFrame, y: np.ndarray, 
        ...
        # Timeout is set globally in __init__, but ensure it's applied here too
        # (set_param is global, so this is just for clarity)
    ...
    X: pd.DataFrame,
     y: np.ndarray) -> List[Dict]:
        ...
        - Limits to top N feature pairs (default 10) instead of trying all pairs
        - Reduces refinement iterations from 5 to 2
        - Early stopping once N good rules found (default 5)
        - Uses shorter Z3 timeout (2 min) for arithmetic learning
        """
        if not self.use_optimization:
            return []

        import time
        arithmetic_start_time = time.time()
    ...
    col for col in X.columns if pd.api.types.is_numeric_dtype(
        ...
        # Performance parameters - tuned for balance between speed and accuracy
        # Original: tried all pairs, 5 refinement iterations, no limit on rules, 5 min timeout
        # Optimized: limit pairs but allow more exploration, keep some refinement, allow more rules
        num_features = len(numeric_cols)
        # Scale max_feature_pairs based on number of features (but cap at reasonable limit)
        # For 5 features: ~10 pairs, for 9 features: ~20 pairs (instead of all 36)
        max_feature_pairs = min(20, max(10, num_features * 2))  # Adaptive: 10-20 pairs based on features
        max_refinement_iterations = 3  # Balance: 3 iterations (was: 5, too aggressive at 2)
        max_arithmetic_rules = 8  # Allow more rules for better coverage (was: 5, too restrictive)
        arithmetic_z3_timeout_ms = 180000  # 3 minutes (balance: was 2 min too short, 5 min too long)

        if self.verbose:
            print(f"\n[Arithmetic Learning] Attempting to learn linear relationships from {len(numeric_cols)} numeric features...")
            print(f"  [Performance] Limiting to top {max_feature_pairs} feature pairs, max {max_arithmetic_rules} rules, {max_refinement_iterations} refinement iterations")
        ...
        # OPTIMIZATION: Prioritize feature pairs based on correlation with target
        # This helps us try the most promising pairs first
        feature_pairs = []
        for i, col1 in enumerate(numeric_cols):
            for j, col2 in enumerate(numeric_cols):
                if i >= j:  # Avoid duplicates
                    continue
                # Calculate simple correlation score (how well the pair separates positives/negatives)
                try:
                    ...
                    if val1_series.isna().all() or val2_series.isna().all():
                        continue
                    # Simple heuristic: correlation with target
                    # For positive examples, compute mean of feature values
                    pos_val1_mean = val1_series[y_series == 1].mean() if (y_series == 1).sum() > 0 else 0
                    pos_val2_mean = val2_series[y_series == 1].mean() if (y_series == 1).sum() > 0 else 0
                    neg_val1_mean = val1_series[y_series == 0].mean() if (y_series == 0).sum() > 0 else 0
                    neg_val2_mean = val2_series[y_series == 0].mean() if (y_series == 0).sum() > 0 else 0
                    # Score: how different are positive vs negative means?
                    score = abs(pos_val1_mean - neg_val1_mean) + abs(pos_val2_mean - neg_val2_mean)
                    feature_pairs.append((score, i, j, col1, col2))
                except:
                    # If scoring fails, use default score
                    feature_pairs.append((0.0, i, j, col1, col2))
        
        # Sort by score (highest first) and take top N
        feature_pairs.sort(reverse=True, key=lambda x: x[0])
        feature_pairs = feature_pairs[:max_feature_pairs]
        
        if self.verbose:
            print(f"  [Performance] Selected top {len(feature_pairs)} feature pairs to try (out of {len(numeric_cols) * (len(numeric_cols) - 1) // 2} possible)")
        ...
        # Try 2-variable combinations first (now limited to top pairs)
        pairs_tried = 0
        for score, i, j, col1, col2 in feature_pairs:
            # Early stopping: if we have enough good rules, stop
            if len(arithmetic_rules) >= max_arithmetic_rules:
                if self.verbose:
                    print(f"  [Early Stopping] Found {len(arithmetic_rules)} good arithmetic rules, stopping")
                break
            
            pairs_tried += 1
            if self.verbose:
                print(f"  [Pair {pairs_tried}/{len(feature_pairs)}] Trying {col1} × {col2} (score={score:.2f})...")
            ...
            # simultaneously
            # Set shorter timeout for arithmetic learning (2 minutes instead of 5)
            opt = Optimize()
            set_param('timeout', arithmetic_z3_timeout_ms)  # 2 minutes for arithmetic learning
            coeff1 = Real('coeff1')
            coeff2 = Real('coeff2')
            threshold = Real('threshold')
            ...
            try:
                ...
                # Check with timeout tracking
                z3_check_start = time.time()
                check_result = opt.check()
                z3_check_time = time.time() - z3_check_start
                if self.verbose and z3_check_time > 10:
                    print(f"    Z3 check took {z3_check_time:.1f}s")
                ...
                if check_result == sat:
                        ...
                        # OPTIMIZATION: Reduced refinement iterations from 5 to 2
                        # Also stop early if no improvement
                        for refine_iter in range(max_refinement_iterations):
                            ...
                            if refine_pos_satisfied:
                                ...
                                refine_check_start = time.time()
                                refine_check_result = refine_opt.check()
                                refine_check_time = time.time() - refine_check_start
                                
                                if refine_check_result == sat:
                                    ...
                                    if refine_precision > best_precision or (refine_precision == best_precision and refine_pos_coverage > best_pos_coverage):
                                        ...
                                        # No improvement - early stop refinement
                                        if self.verbose and refine_iter > 0:
                                            print(f"    Refinement {refine_iter+1}: No improvement, stopping early")
                                        break
                        ...
                        if precision > 0.5 and pos_coverage > 0.3:
                            ...
                            if self.verbose:
                                print(f"  [OK] Learned: {coeff1_val:.4f}*{col1} + {coeff2_val:.4f}*{col2} <= {threshold_val:.4f} "
                                      f"(precision={precision:.2f}, coverage={pos_coverage:.2f}, time={pair_time:.1f}s)")
                            
                            # Early stopping: if we have enough good rules, stop trying more pairs
                            if len(arithmetic_rules) >= max_arithmetic_rules:
                                if self.verbose:
                                    print(f"  [Early Stopping] Found {len(arithmetic_rules)} good arithmetic rules, stopping")
                                break
                            ...
                            if is_multihalfplane and rules_found_for_pair < max_rules_per_pair:
                                ...
                                if len(remaining_pos_indices) >= 3:  # Need at least 3 positives for a rule
                                    ...
                                    if pos_satisfied_2:
                                        ...
                                        opt2_check_start = time.time()
                                        opt2_check_result = opt2.check()
                                        opt2_check_time = time.time() - opt2_check_start
                                        
                                        if opt2_check_result == sat:
                                            model2 = opt2.model()
                ...
                if self.verbose:
                    print(f"  Error learning arithmetic for {col1}, {col2}: {e}")
                continue
            finally:
                # Restore original Z3 timeout after arithmetic learning
                set_param('timeout', self.z3_timeout_ms)
            
            # Early stopping check after each pair
            if len(arithmetic_rules) >= max_arithmetic_rules:
                if self.verbose:
                    print(f"  [Early Stopping] Found {len(arithmetic_rules)} good arithmetic rules, stopping")
                break
        
        # Restore original Z3 timeout
        set_param('timeout', self.z3_timeout_ms)
        
        arithmetic_time = time.time() - arithmetic_start_time
        if self.verbose:
            print(f"\n[Arithmetic Learning] Completed in {arithmetic_time:.1f}s, learned {len(arithmetic_rules)} rules from {pairs_tried} pairs tried")
        
        # Try 3-variable combinations for 3D problems (also limit these)
        # OPTIMIZATION: Limit 3-variable combinations too (max 5 triplets)
        max_3var_combinations = 5
        if len(numeric_cols) >= 3 and len(arithmetic_rules) < max_arithmetic_rules:
            if self.verbose:
                print(f"[Arithmetic Learning] Also trying 3-variable combinations for 3D problems (limited to {max_3var_combinations})...")
            
            triplets_tried = 0
            for i, col1 in enumerate(numeric_cols):
                if len(arithmetic_rules) >= max_arithmetic_rules or triplets_tried >= max_3var_combinations:
                    break
                for j, col2 in enumerate(numeric_cols):
                    if len(arithmetic_rules) >= max_arithmetic_rules or triplets_tried >= max_3var_combinations:
                        break
                    for k, col3 in enumerate(numeric_cols):
                        ...
                        # Set shorter timeout for arithmetic learning
                        opt = Optimize()
                        set_param('timeout', arithmetic_z3_timeout_ms)  # 2 minutes
                        coeff1 = Real('coeff1')
                        coeff2 = Real('coeff2')
                        coeff3 = Real('coeff3')
                        threshold = Real('threshold')
                        ...
                        try:
                            ...
                            check_result_3d = opt.check()
                            if check_result_3d == sat:
                                model = opt.model()
                                coeff1_val = float(model[coeff1].as_fraction())
                                coeff2_val = float(model[coeff2].as_fraction())
                                ...
                                # OPTIMIZATION: Reduced from 5 to 2 iterations
                                best_coeffs = [coeff1_val, coeff2_val, coeff3_val]
                                best_threshold = threshold_val
                                best_precision = 0
                                best_pos_coverage = 0
                                
                                for refine_iter in range(max_refinement_iterations):  # Reduced from 5 to 2
                                ...
                                if precision > 0.5 and pos_coverage > 0.3:
                                    ...
                                    if len(arithmetic_rules) >= max_arithmetic_rules:
                                        if self.verbose:
                                            print(f"  [Early Stopping] Found {len(arithmetic_rules)} good arithmetic rules, stopping 3-variable search")
                                        break
                        except Exception as e:
                            if self.verbose:
