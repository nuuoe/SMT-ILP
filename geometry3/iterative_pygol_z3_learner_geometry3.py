...
class IterativePyGolZ3Learner(BaseEstimator, ClassifierMixin):
    ...
    pygol_rules: List[str],
     X: pd.DataFrame) -> List[Dict]:
        """
        Global structure induction using PyGOL MIE: convert PyGol Prolog rules to internal format with numeric placeholders and extract candidate clauses with thresholds that will be optimized by SMT.
        """
        converted = []
    ...
    X: pd.DataFrame,
     y: np.ndarray) -> List[Dict]:
        """
        Enhanced SMT fitting, instantiation, and scoring: uses Optimize() for threshold optimization with soft constraints for negative examples and blocking constraints for UNSAT rules. Performs SMT fitting per clause by declaring unknown thresholds as SMT variables and asserting training-wide constraints. Instantiation and filtering: if satisfiable, substitute constants from model.
    ...
    *
    ...
    def _check_rule_consistency(self, rules: List[Dict], X: pd.DataFrame) -> List[Dict]:
        """
        Theory consistency checking: checks if rules contradict each other and removes inconsistent rules
        """
        if not rules:
            return []
