...
# Geometry3-specific learner - no external dataset config needed
...
class IterativePyGolZ3Learner(BaseEstimator, ClassifierMixin):
    ...
    def _preprocess_data(self, X: pd.DataFrame):
        ...
        # For IP: Don't preprocess 'example' or 'obj_id' columns (they're for relational structure)
        dataset_type = getattr(self, 'dataset_type', None)
        is_ip = dataset_type == 'ip'
        skip_cols = ['example', 'obj_id'] if is_ip else []
        
        for col in X_processed.columns:
            if col in skip_cols:
                continue  # Don't preprocess relational columns
        ...
        for col in X_processed.columns:
            if col in skip_cols:
                continue  # Don't encode relational columns
    ...
    def _create_initial_background_knowledge(self, X: pd.DataFrame, y: np.ndarray, original_bk_file: Optional[str] = None):
        ...
        # Check if this is an IP problem
        target_pred = getattr(self, 'target_predicate', 'target')
        is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
        
        with open(self.bk_file, 'w') as f:
            # For IP: Copy propagates facts FIRST (before arithmetic operations)
            if is_ip and original_bk_file and os.path.exists(original_bk_file):
                if self.verbose:
                    print(f"  [IP] Preserving relational BK from {original_bk_file}")
                with open(original_bk_file, 'r') as orig:
                    for line in orig:
                        f.write(line)
                f.write("\n% ============================================================\n")
                f.write("% Numerical features and arithmetic for Z3 optimization\n")
                f.write("% ============================================================\n\n")
            ...
            # For IP: Skip 'example' and 'obj_id' columns (they're for relational structure, not features)
            if is_ip:
                invalid_features.extend(['example', 'obj_id'])
            ...
            for idx, (_, row) in enumerate(X.iterrows()):
                # For IP: Use obj_id as example name (relational), otherwise use e_N
                if is_ip and 'obj_id' in row:
                    example_name = row['obj_id']
                else:
    ...
    def _prepare_pygol_examples(self, X: pd.DataFrame, y: np.ndarray):
        ...
        # Check for IP relational format (has 'example' column with relational atoms)
        if 'example' in X.columns:
            # IP relational format: Use examples directly from 'example' column
            pos_examples = X[y == 1]['example'].tolist()
            neg_examples = X[y == 0]['example'].tolist()
            
            # Write relational examples to files
        ...
        # Original tabular format for geometry problems
    ...
    f"Bottom clause generation failed, using direct examples: {e}")
        ...
        def run_pygol():
            try:
                ...
                # IP-specific configuration for relational learning with PI
                is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
                if is_ip:
                    enable_pi = self.dataset_config.get('enable_predicate_invention', False)
                    constants = []  # Empty for IP generalization
                    key_size = 4 if enable_pi else 1
                    max_lits = self.dataset_config.get('max_literals_pi', 4) if enable_pi else effective_max_literals
                    
                    if self.verbose:
                        print(f"  [IP] PyGol config: max_literals={max_lits}, pi={enable_pi}, key_size={key_size}")
                    
                    pygol_model = pygol_learn(
                        Training_pos=Train_P,
                        Training_neg=Train_N,
                        file=bk_file_path,
                        constant_set=constants,  # Empty for IP
                        max_literals=max_lits,
                        max_neg=0,
                        min_pos=1,
                        key_size=key_size,
                        set_chain=True,  # Enable for relational learning
                        pi=enable_pi,  # Enable PI when configured
                        rule_noise_check=True,
                        verbose=self.verbose
                    )
                else:
                    # Original geometry3 configuration
                    pygol_model = pygol_learn(
                        Training_pos=Train_P,
                        Training_neg=Train_N,
                        file=bk_file_path,
    ...
    pygol_rules: List[str],
     X: pd.DataFrame) -> List[Dict]:
        ...
        For IP: Keep relational rules as-is (don't try to convert to numerical format)
        """
        converted = []
        
        # Check if this is IP - if so, keep relational rules
        target_pred = getattr(self, 'target_predicate', 'target')
        is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
        
        if is_ip:
            # For IP: Keep relational rules as-is
            for rule_str in pygol_rules:
                if ':-' in rule_str:
                    converted.append({
                        'pygol_source': rule_str,
                        'type': 'relational',
                        'precision': 0.8,  # Placeholder - will be evaluated
                        'recall': 0.8,
                        'score': 0.8,
                        'f1': 0.8
                    })
        ...
        # For non-IP: Use original conversion logic
        for rule_str in pygol_rules:
            try:
                if ':-' not in rule_str:
                    continue
    ...
    *
    ...
    def fit(self, X: pd.DataFrame, y: np.ndarray):
        ...
        try:
            ...
            self.iteration_history.append({
                ...
                'rules_learned': len(pygol_rules) if 'pygol_rules' in locals() else 0,
                'rules_verified': len(verified_rules) if 'verified_rules' in locals() else 0,
            ...
            if dataset_type == 'geometry':
                ...
                # For IP: Use lower thresholds for relational rules  
                target_pred = getattr(self, 'target_predicate', 'target')
                is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
                
                if is_ip:
                    min_precision = 0.3  # Very low for IP (relational rules)
                    min_recall = 0.01
                    high_precision_threshold = 0.4
                else:
                    min_precision = 0.6
                    min_recall = 0.05
            ...
            # For IP: Keep all rules if filtering is too strict
            target_pred = getattr(self, 'target_predicate', 'target')
            is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
            
            if not balanced_rules:
                if is_ip:
                    # For IP: Keep all rules (don't filter relational rules aggressively)
                    balanced_rules = all_rules
                    if self.verbose:
                        print(f"  [IP] Keeping all {len(balanced_rules)} relational rules (no precision filtering)")
                else:
            ...
            if len(balanced_rules) > max_rules:
                ...
                # For IP: Use lower F1 threshold
                target_pred = getattr(self, 'target_predicate', 'target')
                is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
                min_f1 = 0.01 if is_ip else 0.1
                ...
                # For IP: Don't filter, for others use F1 > 0.05
                target_pred = getattr(self, 'target_predicate', 'target')
                is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
            ...
            # Z3 Refinement for IP: Add numerical constraints to relational rules
            target_pred = getattr(self, 'target_predicate', 'target')
            is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
            use_z3_refinement = self.dataset_config.get('use_optimization', False) if self.dataset_config else False
            
            if is_ip and use_z3_refinement and len(self.learned_rules) > 0:
                if self.verbose:
                    print(f"\n[Z3 Refinement] Attempting to add numerical constraints to {len(self.learned_rules)} relational rules...")
                self.learned_rules = self._refine_relational_rules_with_z3(self.learned_rules, X, y)
            ...
            if self.verbose:
                ...
                target_pred = getattr(self, 'target_predicate', 'target')
                is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
                threshold_msg = f"precision > {min_precision}, recall >= {min_recall}" if 'min_precision' in locals() else "0.6, recall >= 0.05"
                print(f"\n[Rule Filtering] After filtering ({threshold_msg}): {len(self.learned_rules)} rules")
            print(f"\nTop 5 rules:")
    ...
    def predict(self, X: pd.DataFrame):
        ...
        For IP: Evaluate relational rules using propagates facts.
        For others: Use ensemble voting methods.
        """
        # Check if this is IP with relational rules
        target_pred = getattr(self, 'target_predicate', 'target')
        is_ip = target_pred in ['ip1_active', 'ip2_active', 'ip3_active', 'ip3_threshold', 'ip4_high_score']
        
        if is_ip and 'example' in X.columns:
            # IP relational prediction
            return self._predict_relational_ip(X)
        
        # Standard prediction for non-IP
        X_processed = self._preprocess_data(X)
    ...
    def _predict_relational_ip(self, X: pd.DataFrame) -> np.ndarray:
        """
        Predict for IP using relational rules.
        Evaluates rules like: ip1_active(A) :- propagates(A,B), propagates(B,C)
        """
        if not self.learned_rules:
            return np.array([0] * len(X))
        
        # Load propagates facts from BK
        propagates_facts = set()
        original_bk = getattr(self, 'original_bk_file', None)
        if original_bk and os.path.exists(original_bk):
            with open(original_bk, 'r') as f:
                for line in f:
                    if line.strip().startswith('propagates('):
                        # Parse propagates(obj1, obj2).
                        match = re.match(r'propagates\((\w+),\s*(\w+)\)', line.strip().rstrip('.'))
                        if match:
                            propagates_facts.add((match.group(1), match.group(2)))
        ...
        for idx, row in X.iterrows():
            ...
            for rule in self.learned_rules:
                rule_str = rule.get('pygol_source', '')
                rule_vote = 0
                
                # Simple pattern matching for relational rules
                # Rule: ip1_active(A) :- propagates(A,B), propagates(B,A)
                if 'propagates(A,B),propagates(B,A)' in rule_str:
                    # Check if obj has mutual propagation
                    for (obj1, obj2) in propagates_facts:
                        if obj1 == obj_id and (obj2, obj1) in propagates_facts:
                            rule_vote = 1
                            break
                
                # Rule: ip1_active(A) :- propagates(A,B)
                elif 'propagates(A,B)' in rule_str and 'propagates(B,' not in rule_str:
                    # Check if obj propagates to anyone
                    for (obj1, obj2) in propagates_facts:
                        if obj1 == obj_id:
                            rule_vote = 1
                            break
                
                # Rule: ip2_active(A) :- propagates(A,B), propagates(B,C)
                # Also matches: propagates(A,B),propagates(B,C),propagates(C,A) (triangle)
                elif 'propagates(A,B),propagates(B,C)' in rule_str:
                    # Check for 2-hop chain (or triangle if rule has propagates(C,A))
                    has_return = 'propagates(C,A)' in rule_str
                    
                    for (obj1, obj2) in propagates_facts:
                        if obj1 == obj_id:
                            # Check if obj2 propagates to someone
                            for (obj3, obj4) in propagates_facts:
                                if obj3 == obj2:
                                    # If triangle pattern, also check C->A
                                    if has_return:
                                        if (obj4, obj_id) in propagates_facts:
                                            rule_vote = 1
                                            break
                                    else:
                                        # Simple 2-hop chain
                ...
                # Rule: ip3_active(A) :- propagates(A,B), propagates(B,C), propagates(C,D)
                # Also matches triangle: propagates(A,B),propagates(B,C),propagates(C,A)
                elif 'propagates(A,B),propagates(B,C),propagates(C' in rule_str:
                    has_return = 'propagates(C,A)' in rule_str
                    has_cb_ba = 'propagates(C,B),propagates(B,A)' in rule_str
                    
                    # Check for 3-hop chain or triangle
                    for (obj1, obj2) in propagates_facts:
                        if obj1 == obj_id:
                            for (obj3, obj4) in propagates_facts:
                                if obj3 == obj2:
                                    if has_cb_ba:
                                        # Pattern: A->B->C->B->A (4-literal cycle)
                                        if (obj4, obj2) in propagates_facts:  # C->B
                                            if (obj2, obj_id) in propagates_facts:  # B->A
                                                rule_vote = 1
                                                break
                                    elif has_return:
                                        # Triangle: check C->A
                                        ...
                                        # 3-hop chain: check C->D
                                        for (obj5, obj6) in propagates_facts:
                                            if obj5 == obj4:
                                                rule_vote = 1
                                                break
                                if rule_vote:
                        ...
                        if rule_vote:
                            break
                # Rule: ip3_threshold patterns - A->B->D->A or A->B->D->C->A
                elif 'propagates(A,B),propagates(B,D)' in rule_str:
                    has_da = 'propagates(D,A)' in rule_str
                    has_dc_ca = 'propagates(D,C),propagates(C,A)' in rule_str
                    ...
                    for (obj1, obj2) in propagates_facts:
                        if obj1 == obj_id:
                            for (obj3, obj4) in propagates_facts:
                                if obj3 == obj2:  # B->D
                                    if has_dc_ca:
                                        # Pattern: A->B->D->C->A (4-literal square)
                                        for (obj5, obj6) in propagates_facts:
                                            if obj5 == obj4:  # D->C
                                                if (obj6, obj_id) in propagates_facts:  # C->A
                                                    rule_vote = 1
                                                    break
                                        if rule_vote:
                                            break
                                    elif has_da:
                                        # Pattern: A->B->D->A (3-literal triangle)
                                        if (obj4, obj_id) in propagates_facts:  # D->A
                                            rule_vote = 1
                                            break
                                if rule_vote:
                                    break
                ...
                # Apply numerical constraint if present
                if rule_vote == 1 and 'numerical_constraint' in rule:
                    nc = rule['numerical_constraint']
                    feat_val = row[nc['feature']]
                    thresh = nc['threshold']
                    
                    # Check if numerical constraint is satisfied
                    if nc['operation'] == '>':
                        if feat_val <= thresh:
                            rule_vote = 0  # Doesn't satisfy numerical constraint
                    elif nc['operation'] == '<':
                        if feat_val >= thresh:
                            rule_vote = 0
                
                # If this rule matched, set vote and stop checking other rules
                if rule_vote == 1:
                    vote = 1
                    break
    ...
    def _refine_relational_rules_with_z3(self, rules: List[Dict], X: pd.DataFrame, y: np.ndarray) -> List[Dict]:
        """
        Refine relational rules by adding Z3-optimized numerical constraints.
        
        Uses train/validation split to prevent overfitting:
        - Optimize threshold on train subset
        - Evaluate on validation subset
        - Only keep if validation improves
        
        Example:
        Base: ip2_active(A) :- propagates(A,B), propagates(B,C), propagates(C,A)
        Refined: ip2_active(A) :- propagates(A,B), propagates(B,C), propagates(C,A), max_out_influence(A) > 4.2
        """
        from z3 import Optimize, Real, If, And
        from sklearn.model_selection import train_test_split
        
        # Split into train/validation to prevent overfitting
        if len(X) < 10:
            # Too small to split - skip Z3 refinement
            return rules
        
        try:
            # Use larger validation split (50%) for more reliable threshold selection
            # This prevents overfitting to a small validation set
            X_train, X_val, y_train, y_val = train_test_split(
                X, y, test_size=0.5, random_state=42, stratify=y
            )
        except:
            # If split fails (e.g., too few samples), skip refinement
            return rules
        
        numeric_features = ['score', 'num_propagates_out', 'max_out_influence', 'avg_out_influence', 'total_neighbor_score', 'max_chain_score']
        refined_rules = []
        
        for rule in rules:
            # Skip if not relational
            if rule.get('type') != 'relational':
                refined_rules.append(rule)
                continue
            
            # Evaluate base rule on VALIDATION set
            base_acc_val = self._evaluate_refined_rule(rule, X_val, y_val)
            best_rule = rule.copy()
            best_acc_val = base_acc_val
            
            # Try adding each numerical feature
            for feat in numeric_features:
                if feat not in X.columns:
                    continue
                
                try:
                    # Use Z3 to find optimal threshold on TRAIN subset
                    opt_threshold = self._optimize_numerical_threshold_for_relational(rule, X_train, y_train, feat)
                    
                    if self.verbose:
                        print(f"  Trying {feat}: threshold={opt_threshold}")
                    
                    if opt_threshold is not None:
                        # Create refined rule
                        refined = rule.copy()
                        refined['numerical_constraint'] = {
                            'feature': feat,
                            'operation': '>',
                            'threshold': opt_threshold
                        }
                        
                        # Evaluate refined rule on VALIDATION subset (not train!)
                        refined_acc_val = self._evaluate_refined_rule(refined, X_val, y_val)
                        
                        if self.verbose:
                            print(f"    Val: Base {base_acc_val:.3f}, Refined {refined_acc_val:.3f}")
                        
                        # Require meaningful improvement on VALIDATION (at least 5% for F1 score)
                        # AND ensure it doesn't hurt base accuracy (must be at least as good)
                        # This prevents Z3 from making things worse
                        if refined_acc_val > best_acc_val + 0.05 and refined_acc_val >= base_acc_val:
                            best_rule = refined
                            best_acc_val = refined_acc_val
                            if self.verbose:
                                print(f"  Refined rule with {feat} > {opt_threshold:.3f}: Val F1 {base_acc_val:.3f} -> {refined_acc_val:.3f}")
                except Exception as e:
                    if self.verbose:
                        import traceback
                        print(f"  Failed to refine with {feat}: {e}")
                        traceback.print_exc()
                    continue
    ...
    def _optimize_numerical_threshold_for_relational(self, rule: Dict, X: pd.DataFrame, y: np.ndarray, feature: str) -> Optional[float]:
        """Use Z3 to find optimal threshold for a numerical feature"""
        from z3 import Optimize, Real, If
        
        # Get examples that satisfy the relational pattern
        relational_matches = self._get_relational_matches(rule, X)
        
        if len(relational_matches) == 0:
            return None
        
        # Get feature values for matched examples
        y_series = pd.Series(y, index=X.index)
        matched_positives = [idx for idx in relational_matches if y_series[idx] == 1]
        matched_negatives = [idx for idx in relational_matches if y_series[idx] == 0]
        
        if len(matched_positives) == 0:
            return None
        
        # Use Z3 to optimize threshold
        opt = Optimize()
        threshold = Real('threshold')
        ...
        # Set more conservative bounds: threshold should be at least median of positives
        # This prevents overly permissive thresholds
        pos_values = [float(X.loc[idx, feature]) for idx in matched_positives]
        if len(pos_values) > 0:
            pos_median = sorted(pos_values)[len(pos_values) // 2]
            # Threshold should be at least 25th percentile of positives
            pos_25th = sorted(pos_values)[len(pos_values) // 4] if len(pos_values) >= 4 else pos_median
            opt.add(threshold >= pos_25th)  # More conservative lower bound
        else:
            opt.add(threshold >= feat_min)
        
        opt.add(threshold <= feat_max)
        
        # Maximize positives above threshold, minimize negatives above threshold
        pos_constraints = []
        for idx in matched_positives:
            val = float(X.loc[idx, feature])
            pos_constraints.append(If(val > threshold, 1, 0))
        
        neg_constraints = []
        for idx in matched_negatives:
            val = float(X.loc[idx, feature])
            neg_constraints.append(If(val > threshold, 1, 0))
        
        # Create Z3 sum expressions (not Python int)
        if len(pos_constraints) > 0:
            from z3 import Sum
            pos_score = Sum(pos_constraints) if len(pos_constraints) > 1 else pos_constraints[0]
            opt.maximize(pos_score)
        
        if len(neg_constraints) > 0:
            from z3 import Sum
            neg_score = Sum(neg_constraints) if len(neg_constraints) > 1 else neg_constraints[0]
            opt.minimize(neg_score)
        
        if opt.check():
            model = opt.model()
            thresh_val = model[threshold]
            
            # Handle different Z3 result types
            try:
                # Try as rational
                if hasattr(thresh_val, 'as_fraction'):
                    frac = thresh_val.as_fraction()
                    return float(frac.numerator) / float(frac.denominator)
                elif hasattr(thresh_val, 'numerator_as_long'):
                    return float(thresh_val.numerator_as_long()) / float(thresh_val.denominator_as_long())
                else:
                    # Try as decimal/float
                    return float(thresh_val.as_decimal(10).replace('?', ''))
            except:
    ...
    def _get_relational_matches(self, rule: Dict, X: pd.DataFrame) -> List:
        """Get examples that match the relational pattern"""
        # Use same logic as _predict_relational_ip
        matches = []
        
        # Load propagates facts
        bk_file = getattr(self, 'original_bk_file', None)
        if not bk_file or not os.path.exists(bk_file):
            return matches
        
        propagates_facts = []
        with open(bk_file, 'r') as f:
            for line in f:
                if line.startswith('propagates('):
                    match = re.match(r'propagates\((\w+),\s*(\w+)\)', line.strip().rstrip('.'))
                    if match:
                        propagates_facts.append((match.group(1), match.group(2)))
        ...
        for idx, row in X.iterrows():
            ...
            # Check relational pattern (must handle ALL patterns PyGol might learn)
            if 'propagates(A,B),propagates(B,A)' in rule_str:
                # 2-literal: mutual propagation
                for (obj1, obj2) in propagates_facts:
                    if obj1 == obj_id and (obj2, obj1) in propagates_facts:
                        matches.append(idx)
                        break
            elif 'propagates(A,B),propagates(B,C),propagates(C,D),propagates(D,A)' in rule_str:
                # 4-literal: square/cycle pattern
                for (obj1, obj2) in propagates_facts:
                    if obj1 == obj_id:
                        for (obj3, obj4) in propagates_facts:
                            if obj3 == obj2:
                                for (obj5, obj6) in propagates_facts:
                                    if obj5 == obj4:
                                        for (obj7, obj8) in propagates_facts:
                                            if obj7 == obj6 and obj8 == obj_id:
                                                matches.append(idx)
                                                break
                                        if idx in matches:
                                            break
                        ...
                        if idx in matches:
                            break
            elif 'propagates(A,B),propagates(B,C),propagates(C,A)' in rule_str:
                # 3-literal: triangle pattern
                for (obj1, obj2) in propagates_facts:
                    if obj1 == obj_id:
                        for (obj3, obj4) in propagates_facts:
                            if obj3 == obj2 and (obj4, obj1) in propagates_facts:
                                matches.append(idx)
                                break
                        if idx in matches:
                            break
            elif 'propagates(A,B),propagates(B,C),propagates(C,B),propagates(B,A)' in rule_str:
                # 4-literal: A->B->C->B->A pattern (complex cycle)
                for (obj1, obj2) in propagates_facts:
                    if obj1 == obj_id:
                        for (obj3, obj4) in propagates_facts:
                            if obj3 == obj2:  # B->C
                                for (obj5, obj6) in propagates_facts:
                                    if obj5 == obj4 and obj6 == obj2:  # C->B
                                        if (obj2, obj_id) in propagates_facts:  # B->A
                                            matches.append(idx)
                                            break
                                if idx in matches:
                        ...
                        if idx in matches:
                            break
            elif 'propagates(A,B),propagates(B,C),propagates(C,B)' in rule_str and 'propagates(B,A)' in rule_str:
                # Alternative 4-literal pattern: A->B->C->B->A (same pattern, different order in string)
                for (obj1, obj2) in propagates_facts:
                    if obj1 == obj_id:
                        for (obj3, obj4) in propagates_facts:
                            if obj3 == obj2:  # B->C
                        ...
                        if idx in matches:
                            break
            elif 'propagates(A,B),propagates(B,D),propagates(D,A)' in rule_str:
                # 3-literal: A->B->D->A triangle pattern (different variable names)
                for (obj1, obj2) in propagates_facts:
                    if obj1 == obj_id:
                        for (obj3, obj4) in propagates_facts:
                            if obj3 == obj2:  # B->D
                        ...
                        if idx in matches:
                            break
            elif 'propagates(A,B),propagates(B,D),propagates(D,C),propagates(C,A)' in rule_str:
                # 4-literal: A->B->D->C->A square pattern
                for (obj1, obj2) in propagates_facts:
                    if obj1 == obj_id:
                        for (obj3, obj4) in propagates_facts:
                            if obj3 == obj2:  # B->D
    ...
    def _evaluate_refined_rule(self, rule: Dict, X: pd.DataFrame, y: np.ndarray) -> float:
        """
        Evaluate accuracy of a refined rule (relational + numerical)
        Uses F1 score instead of just precision to balance precision/recall
        This prevents overfitting to training data
        """
        matches = self._get_relational_matches(rule, X)
        
        if 'numerical_constraint' in rule:
            # Apply numerical filter
            feat = rule['numerical_constraint']['feature']
            thresh = rule['numerical_constraint']['threshold']
            matches = [idx for idx in matches if X.loc[idx, feat] > thresh]
        ...
        # Create predictions: 1 for matches, 0 for non-matches
        y_pred = np.zeros(len(X))
        for idx in matches:
            y_pred[X.index.get_loc(idx)] = 1
        
        try:
            f1 = f1_score(y, y_pred, zero_division=0)
            return f1
        except:
            # Fallback to precision if F1 fails
            correct = sum(1 for idx in matches if y_series[idx] == 1)
            return correct / len(matches) if len(matches) > 0 else 0.0
